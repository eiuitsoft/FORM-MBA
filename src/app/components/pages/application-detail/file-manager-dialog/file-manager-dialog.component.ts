        ...
        <div
          ...
          <div class="bg-gray-50 px-6 py-4">
            ...
            <div class="flex items-center justify-center py-8">
              <svg
                class="animate-spin h-8 w-8 text-[#a68557]"
              ...
              <label
                ...
                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                  ...
                  <p class="mb-2 text-sm text-gray-500">
                    <span class="font-semibold">{{ 'FILE_DIALOG.CLICK_TO_UPLOAD' | translate }}</span>
                  </p>
            ...
            <div class="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <div class="flex items-center justify-between mb-3">
                ...
                <span class="text-xs text-gray-500"
                  >{{ pendingFiles.length }} {{ 'FILE_DIALOG.FILES' | translate }}</span
                >
              </div>
              <div class="space-y-2">
            ...
            <div class="mb-4">
              <button
                ...
                <span
                  ...
                  {{ pendingFiles.length > 0 ? '(' + pendingFiles.length + ')' : '' }}</span
                >
                }
              </button>
            </div>
...
export class FileManagerDialogComponent implements OnChanges {
    ...
    for (const file of selectedFiles) {
      if (file.size > 5 * 1024 * 1024) {
        this._alertService.error(
          this._translate.instant('FILE_DIALOG.ERROR'),
          `${file.name} ${this._translate.instant('FILE_DIALOG.FILE_SIZE_LIMIT')}`
        );
        continue;
      }
      ...
      if (!allowedTypes.includes(file.type)) {
        this._alertService.error(
          this._translate.instant('FILE_DIALOG.ERROR'),
          `${file.name} ${this._translate.instant('FILE_DIALOG.FILE_INVALID_TYPE')}`
        );
        continue;
      }
    ...
    this._mbaService.uploadAdmissionFiles(formData).subscribe({
      next: (result: any) => {
        ...
        if (uploadedData.length > 0) {
          ...
          this.onSave.emit(this.files);
          this.pendingFiles = [];
          this._alertService.successMin(
            `${this._translate.instant('FILE_DIALOG.UPLOAD_SUCCESS')} ${uploadedFiles.length} ${this._translate.instant(
              'FILE_DIALOG.FILES'
            )}`
          );
        } else {
          this._alertService.error(
            this._translate.instant('FILE_DIALOG.ERROR'),
            this._translate.instant('FILE_DIALOG.UPLOAD_FAILED')
          );
        }
      },
      error: () => {
        ...
        this._alertService.error(
          this._translate.instant('FILE_DIALOG.ERROR'),
          this._translate.instant('FILE_DIALOG.UPLOAD_ERROR')
        );
      }
    });
  }
  ...
  async deleteUploadedFile(index: number): Promise<void> {
    ...
    if (confirmed && file.fileFullPath) {
      this._mbaService.removeFile(file.fileFullPath).subscribe({
        next: (result) => {
          if (result.success) {
            ...
            this._alertService.error(
              this._translate.instant('FILE_DIALOG.ERROR'),
              result.message || this._translate.instant('FILE_DIALOG.DELETE_FAILED')
            );
          }
        },
        error: () => {
          this._alertService.error(
            this._translate.instant('FILE_DIALOG.ERROR'),
            this._translate.instant('FILE_DIALOG.DELETE_ERROR')
          );
        }
      });
    }
    ...
    if (file.fileFullPath) {
      this._mbaService.downloadFile(file.fileFullPath).subscribe({
        ...
        error: () => {
          this._alertService.error(
            this._translate.instant('FILE_DIALOG.ERROR'),
            this._translate.instant('FILE_DIALOG.DOWNLOAD_ERROR')
          );
        }
      });
    }
