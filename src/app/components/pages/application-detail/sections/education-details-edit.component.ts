...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, EducationFileManagerDialogComponent, TranslatePipe],
      ...
      <div class="p-6 bg-gray-50 space-y-6">
          ...
          <div [formGroupName]="i" class="mb-4 pb-4 border-b border-gray-300 last:border-b-0">
            <div class="flex justify-between items-center mb-3">
              <p class="text-sm font-bold text-[#a68557] uppercase">
                @if (i === 0) { {{ 'EDUCATION_DETAILS.UNDERGRAD_TITLE' | translate }} } @else if (i === 1) {
                {{ 'EDUCATION_DETAILS.SECOND_DEGREE' | translate }} } @else { {{ i + 1
                }}{{ 'EDUCATION_DETAILS.OTHER_DEGREE' | translate }} }
              </p>
              @if (i > 1) {
              <button
                type="button"
                (click)="removeUndergraduate(i)"
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-3">
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1"
                  >{{ 'EDUCATION_DETAILS.UNIVERSITY' | translate }} @if (i === 0) {
              ...
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1"
                  >{{ 'EDUCATION_DETAILS.COUNTRY' | translate }} @if (i === 0) {
                  <span class="text-red-600">*</span> }</label
                >
                <select
              ...
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1"
                  >{{ 'EDUCATION_DETAILS.MAJOR' | translate }} @if (i === 0) {
                  <span class="text-red-600">*</span> }</label
                >
                <input
                  type="text"
              ...
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1"
                  >{{ 'EDUCATION_DETAILS.GRAD_YEAR' | translate }} @if (i === 0) {
                  <span class="text-red-600">*</span> }</label
                >
                <input
                  type="number"
              ...
              <div class="md:col-span-2">
                <label class="block text-xs font-medium text-gray-600 mb-1"
                  >{{ 'EDUCATION_DETAILS.LANGUAGE' | translate }} @if (i === 0) {
                  <span class="text-red-600">*</span> }</label
                >
                <select
            ...
            <div>
              <label class="block text-xs font-medium text-gray-700 mb-2">{{
                'EDUCATION_DETAILS.UPLOAD_CERT' | translate
              }}</label>
              <button
                type="button"
                (click)="openFileManager('undergraduate', i)"
                [class]="
                  undergraduateFiles && undergraduateFiles[i] && undergraduateFiles[i].length > 0
                ...
                <span
                  ...
                  {{
                    undergraduateFiles && undergraduateFiles[i] && undergraduateFiles[i].length > 0
                      ? undergraduateFiles[i].length + ' ' + ('COMMON.FILE_SELECTED' | translate)
        ...
        <div formArrayName="postgraduates">
          ...
          <div [formGroupName]="i" class="mb-4 pb-4 border-b border-gray-300 last:border-b-0">
            <div class="flex justify-between items-center mb-3">
              <p class="text-sm font-bold text-[#a68557] uppercase">
                @if (i === 0) { {{ 'EDUCATION_DETAILS.POSTGRADUATE_TITLE' | translate }} } @else { {{ i + 1
                }}{{ 'EDUCATION_DETAILS.OTHER_POSTGRAD' | translate }} }
              </p>
              @if (i > 0) {
              <button
                type="button"
                (click)="removePostgraduate(i)"
                class="text-red-600 hover:text-red-800 text-sm font-medium">
                ✕ {{ 'COMMON.REMOVE' | translate }}
              </button>
              }
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-3">
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">{{
                  'EDUCATION_DETAILS.UNIVERSITY' | translate
                }}</label>
                <input
                  type="text"
                  formControlName="university"
              ...
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">{{
                  'EDUCATION_DETAILS.COUNTRY' | translate
                }}</label>
                <select
                  formControlName="countryId"
                  class="block w-full rounded border-gray-300 shadow-sm focus:border-[#a68557] focus:ring-[#a68557] px-3 py-2 text-sm">
              ...
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">{{
                  'EDUCATION_DETAILS.MAJOR' | translate
                }}</label>
                <input
                  type="text"
                  formControlName="major"
              ...
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">{{
                  'EDUCATION_DETAILS.GRAD_YEAR' | translate
                }}</label>
                <input
                  type="number"
                  formControlName="graduationYear"
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-3">
              ...
              <div class="md:col-span-3">
                <label class="block text-xs font-medium text-gray-600 mb-1">{{
                  'EDUCATION_DETAILS.THESIS' | translate
                }}</label>
                <input
                  type="text"
                  formControlName="thesisTitle"
                  class="block w-full rounded border-gray-300 shadow-sm focus:border-[#a68557] focus:ring-[#a68557] px-3 py-2 text-sm" />
              </div>
            </div>
            ...
            <div>
              ...
              <button
                type="button"
                (click)="openFileManager('postgraduate', i)"
                [class]="
                  postgraduateFiles && postgraduateFiles[i] && postgraduateFiles[i].length > 0
                    ? 'w-full flex items-center justify-center px-4 py-3 border-2 border-dashed border-green-400 rounded bg-green-50 hover:border-green-500 hover:bg-green-100 transition-colors'
                    : 'w-full flex items-center justify-center px-4 py-3 border-2 border-dashed border-blue-300 rounded bg-blue-50 hover:border-blue-400 hover:bg-blue-100 transition-colors'
                ">
                <svg
                ...
                <span
                  ...
                  {{
                    postgraduateFiles && postgraduateFiles[i] && postgraduateFiles[i].length > 0
                      ? postgraduateFiles[i].length + ' ' + ('COMMON.FILE_SELECTED' | translate)
                      : ('COMMON.FILE_UPLOAD' | translate)
                  }}
                </span>
              </button>
